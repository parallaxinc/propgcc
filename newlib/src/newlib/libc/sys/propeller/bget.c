...
#include <stdio.h>
#ifdef __nuon__
#include <nuon/nuonos.h>

KTMUTEX *alloc_mutex;  /* initialized in __initlib */
#endif
...
static void *(*acqfcn) _((bufsize size)) = sbrk;
static void (*relfcn) _((void *buf)) = NULL;

static bufsize exp_incr = 4096;	      /* Expansion block size */
...
#ifdef __nuon__
    /* protect us from other threads */
    _KtMutexLock(alloc_mutex);
#endif
...
{
    void *mem;
    unsigned int heapsz = (unsigned int)heap_size;

    unsigned int startval;
    // Find out where the heap is now (in case someone used sbrk before
    // us). This also guarantees that there will be a call to sbrk
    // before any use of _set_heapmax.
    startval = (unsigned int)sbrk(0);  // find out where the heap is now
    if(heapsz <= 16){
        heapsz = _set_heapmax(0);
        _set_heapmax(heapsz);	// Don't really want it changed
        heapsz -= startval;
    }
    mem = sbrk(heapsz);
    if(mem == 0) return;
