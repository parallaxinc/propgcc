...
extern unsigned int _load_start_i2c_driver_bootcog[];
...
I2C *i2cBootOpen(void)
{
	if (!_boot_i2c) {
		...
		if ((_boot_i2c_data.cog = cognew(_load_start_i2c_driver_bootcog, &init)) < 0)
			return NULL;
...
void *i2cBootBuffer(void)
{
	...
	return (void *)_load_start_i2c_driver_bootcog;
}
...
int cognewFromBootEeprom(void *code, size_t codeSize, void *param)
{
    ...
    cogbuf = (void *)_load_start_i2c_driver_bootcog;
